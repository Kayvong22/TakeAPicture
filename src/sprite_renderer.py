...
if __name__ == "__main__":
    # Example 1: Basic rendering with original sprite preserved
    renderer = SpriteRenderer("character_template.json")
    ...
    # Render and save (pass original sprite path to preserve unclassified pixels)
    renderer.save_render(colors, "output_sprite.png", scale=10, 
                        original_image_path="original_sprite.png")
    ...
    sprites = renderer.render_multiple(color_variations, 
                                      original_image_path="original_sprite.png")
    
    # Save each variation
    for i, sprite in enumerate(sprites):
        scaled = sprite.resize((sprite.width * 10, sprite.height * 10), Image.NEAREST)
        scaled.save(f"character_variation_{i+1}.png")
    
    print(f"Rendered {len(sprites)} sprite variations!")
    
    # Example 3: Programmatic color generation
    import colorsys
    
    # Generate rainbow hair variations
    for hue in range(0, 360, 30):
        rgb = colorsys.hsv_to_rgb(hue / 360, 0.8, 0.9)
        hair_color = f"#{int(rgb[0]*255):02x}{int(rgb[1]*255):02x}{int(rgb[2]*255):02x}"
        ...
        renderer.save_render(colors, f"rainbow_hair_{hue}.png", scale=8,
                           original_image_path="original_sprite.png")